...
Each of these — `AssistantCloud(...)`, `CloudThreadList(...)`, `AISDKThread(...)` — returns a [`ResourceElement`](/tap/docs/tap/resources#resourceelements): a simple `{ hook, args }` object that carries both the implementation and its configuration. Users can swap, compose, and nest them freely.
